create database a0222i1_database_common_functions;
use a0222i1_database_common_functions;
...
select c.`name` as class_name, avg(`point`) as diem_trung_binh
from student s
join class c on s.class_id = c.id
group by c.id;
...
select s.`name` as ho_va_ten, s.birthday as ngaysinh
from student s
union
